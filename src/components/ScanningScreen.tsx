...
function statusIcon(log: ScanLog): React.ReactNode {
  if (log.status === 'queued')  return <span className="text-muted-foreground">·</span>;
  if (log.status === 'probing') return <RiLoader4Line className="w-3 h-3 text-indigo animate-spin shrink-0" />;
  if (!log.result) return <span className="text-muted-foreground">-</span>;
  ...
  switch (log.result.status) {
    ...
    case 'not_found':  return <span className="text-muted-foreground font-mono text-xs">-</span>;
    default:           return <span className="text-amber font-mono text-xs">?</span>;
  }
}

function statusLabel(log: ScanLog): React.ReactNode {
  if (log.status === 'queued')  return <span className="text-muted-foreground">queued</span>;
  ...
  switch (log.result.status) {
    ...
    case 'not_found':  return <span className="text-muted-foreground">not found</span>;
  ...
  return (
    <div className="min-h-screen w-full bg-linear-to-br from-background via-background to-muted/35 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-3xl items-center justify-center">
        <div className="w-full rounded-4xl border border-border/70 bg-card/90 p-4 shadow-lg backdrop-blur sm:p-6">
          ...
          <div className="overflow-hidden rounded-3xl border border-border/70 bg-background/75 shadow-sm">
            ...
            <div className="border-t border-border/70 px-4 pb-4 pt-0 sm:px-5">
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-border/80">
                <div
                  className="h-full rounded-full bg-linear-to-r from-indigo to-green transition-all duration-500 ease-out"
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
